...
Following the EDA Guide, this project answers:
...
![Provider Type Distribution](graphs/g1_provider_type.png)

> **Insight:** Supermarkets (26.2%) are the most common food providers, closely followed by Grocery Stores (25.6%). All four provider types are roughly equally distributed, indicating a diverse ecosystem of food donors.

---

### Graph 2 — Claim Status Distribution
![Claim Status](graphs/g2_claim_status.png)

> **Insight:** Claims are almost equally split — 33.9% Completed, 33.6% Cancelled, 32.5% Pending. The high cancellation rate (33.6%) highlights a significant opportunity to improve the claim completion process through better matching and logistics.

---

### Graph 3 — Food Type Distribution
![Food Type](graphs/g3_food_type.png)

> **Insight:** Food listings are nearly equally split between Vegetarian (33.6%), Vegan (33.4%), and Non-Vegetarian (33.0%), indicating a well-balanced food supply across dietary categories.

---

### Graph 4 — Meal Type Distribution
![Meal Type](graphs/g4_meal_type.png)

> **Insight:** Breakfast (254) has the highest representation in food listings, followed closely by Snacks (253), Lunch (248), and Dinner (245). The distribution is very balanced across all meal types.

---

### Graph 5 — Top 10 Most Common Food Items
![Top Foods](graphs/g5_top_foods.png)

> **Insight:** Rice (114 listings) is the most donated food item, followed by Soup (111) and Salad (105). Staple foods dominate donations, which aligns with practical food redistribution needs.

---

### Graph 6 — Receiver Type Distribution
![Receiver Type](graphs/g6_receiver_type.png)

> **Insight:** NGOs (274) are the largest category of food receivers, followed by Charities (263) and Shelters (246). This shows that organized institutions handle most of the food claims, ensuring structured distribution.

---

### Graph 7 — Food Quantity Distribution
![Quantity Distribution](graphs/g7_quantity_dist.png)

> **Insight:** Food quantities are fairly uniformly distributed between 1 and 50 units, with a mean of 25.79 and median of 26.0. The distribution is slightly bimodal, suggesting two common donation patterns — small and large batches.

---

### Graph 8 — Monthly Claims by Status
![Monthly Claims](graphs/g8_monthly_claims.png)

> **Insight:** Claims activity is concentrated in March 2025, which is the only month in the dataset. The completed, cancelled, and pending claims are evenly distributed throughout the month.

---

### Graph 9 — Top 10 Cities by Providers
![Top Cities](graphs/g9_top_cities.png)

> **Insight:** Provider distribution across cities is highly diverse — the top city has only 3 providers, confirming that no single city dominates. This indicates the system serves a wide geographic area.

---

### Graph 10 — Food Type vs Meal Type Heatmap
![Heatmap](graphs/g10_heatmap.png)

> **Insight:** The heatmap shows a fairly uniform distribution of food types across meal types. Vegetarian Breakfast (89 listings) and Vegan Snacks (92 listings) appear slightly more frequently, suggesting morning and snack-time donations skew plant-based.
...
LIMIT 10;
```
**Purpose:** Identifies geographic hotspots of food donation activity.
...
```
**Purpose:** Determines which category of provider donates the most food by volume.
...
LIMIT 10;
```
**Purpose:** Identifies the most active food receivers for partnership focus.
...
LIMIT 10;
```
**Purpose:** Maps food supply distribution across cities.
...
ORDER BY Count DESC;
```
**Purpose:** Understands dietary diversity in food donations.
...
LIMIT 10;
```
**Purpose:** Reveals which food items are in highest demand.
...
LIMIT 10;
```
**Purpose:** Recognizes the most impactful food providers.
...
ORDER BY Total DESC;
```
**Purpose:** Quantifies claim success rate for performance monitoring.
...
```
**Purpose:** Helps calibrate donation sizes per receiver category.
...
ORDER BY Total_Claims DESC;
```
**Purpose:** Identifies which meals are in highest demand.
...
LIMIT 10;
```
**Purpose:** Ranks providers by total donation contribution.
...
LIMIT 1;
```
**Purpose:** Identifies the most food-abundant city in the system.
...
LIMIT 15;
```
**Purpose:** Urgency queue — items that need to be claimed immediately.
...
```
**Purpose:** Identifies underperforming providers needing outreach.
...
LIMIT 10;
```
**Purpose:** Finds cities with active supply AND demand — best distribution opportunities.
...
> Using `INSERT OR REPLACE INTO` or `to_sql(if_exists='append')` ensures idempotent data loading with no duplicate key errors.
...
│   └── g10_heatmap.png
│
└── 🗄️ food_wastage.db               # SQLite database (auto-generated)
```
...
1. Upload all CSV files to the Colab session
2. Open `food_wastage_analysis.ipynb`
3. Run all cells
...
| Documentation / README / Insights | This comprehensive README + notebook documentation | 10 |
...
- [ ] **Geolocation mapping** — Show food providers and receivers on an interactive map
